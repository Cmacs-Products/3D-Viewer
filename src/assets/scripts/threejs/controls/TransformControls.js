...
THREE.TransformControls = function (camera, domElement) {
    ...
    this.pointerMove = function (pointer) {
        ...
        if (mode === 'translate') {
            ...
            if (Three.Gui.positionSliderUpdate) {
                offset.setX(Three.Gui.positionDelta.x);
                offset.setY(Three.Gui.positionDelta.y);
                offset.setZ(Three.Gui.positionDelta.z);
                //positionStart.set(object.position.x, object.position.y, object.position.z);
            } else {
                if (space === 'local' && axis !== 'XYZ') {
            ...
            // if (axis == "X") {
            //     $("#positionXSlider").kendoSlider('value', Math.round(offset.x));
            // } else if (axis == "Y") {
            //     $("#positionYSlider").kendoSlider('value', Math.round(offset.y));
            // } else {
            //     $("#positionZSlider").kendoSlider('value', Math.round(offset.z));
            // }
            ...
            // Apply scale
            if (Three.Gui.scaleSliderUpdate) {
                scaleStart.set(1, 1, 1);
                object.scale.copy(scaleStart).multiply(Three.Gui.scaleDelta)
            } else {
            ...
            // if (axis == "X") {
            //     if (Three.Gui.scaleSliderUpdate) {
            //         $("#scaleXSlider").kendoSlider('value', Math.round(Three.Gui.scaleDelta.x));
            //     } else {
            //         $("#scaleXSlider").kendoSlider('value', Math.round(_tempVector.x));
            //     }
            // } else if (axis == "Y") {
            //     if (Three.Gui.scaleSliderUpdate) {
            //         $("#scaleYSlider").kendoSlider('value', Math.round(Three.Gui.scaleDelta.y));
            //     } else {
            //         $("#scaleYSlider").kendoSlider('value', Math.round(_tempVector.y));
            //     }
            // } else {
            //     if (Three.Gui.scaleSliderUpdate) {
            //         $("#scaleZSlider").kendoSlider('value', Math.round(Three.Gui.scaleDelta.z));
            //     } else {
            //         $("#scaleZSlider").kendoSlider('value', Math.round(_tempVector.z));
            //     }
            // }
            ...
            if (axis === 'X' || axis === 'Y' || axis === 'Z') {
                ...
                var _tempVector3 = _tempVector.cross(new THREE.Vector3(1, 1, 1)).normalize();
                if (isNaN(_tempVector3.x) && isNaN(_tempVector3.y) && isNaN(_tempVector3.z)) {
                    _tempVector3.setX(0);
                    _tempVector3.setY(0);
                    _tempVector3.setZ(0);
                }
                rotationAngle = _tempVector2.dot(_tempVector3) * ROTATION_SPEED;
            }
            ...
            if (space === 'local') {
                ...
                if (Three.Gui.rotationSliderUpdate) {
                    object.quaternion.copy(_tempQuaternion.setFromAxisAngle(rotationAxis, Three.Gui.rotationAngle));
                    object.quaternion.multiply(object.quaternion);
                } else {
                    object.quaternion.copy(_tempQuaternion.setFromAxisAngle(rotationAxis, rotationAngle));
                    object.quaternion.multiply(quaternionStart);
                }
            ...
            // if (axis == "X") {
            //     if (Three.Gui.rotationSliderUpdate) {
            //         $("#rotationXSlider").kendoSlider('value', Math.round(Three.Gui.rotationAngle * (180 / Math.PI)));
            //     }
            //     else {
            //         $("#rotationXSlider").kendoSlider('value', Math.round(rotationAngle * (180 / Math.PI)));
            //     }

            // } else if (axis == "Y") {
            //     if (Three.Gui.rotationSliderUpdate) {
            //         $("#rotationYSlider").kendoSlider('value', Math.round(Three.Gui.rotationAngle * (180 / Math.PI)));
            //     } else {
            //         $("#rotationYSlider").kendoSlider('value', Math.round(rotationAngle * (180 / Math.PI)));
            //     }

            // } else {
            //     if (Three.Gui.rotationSliderUpdate) {
            //         $("#rotationZSlider").kendoSlider('value', Math.round(Three.Gui.rotationAngle * (180 / Math.PI)));
            //     }
            //     else {
            //         $("#rotationZSlider").kendoSlider('value', Math.round(rotationAngle * (180 / Math.PI)));
            //     }
...
THREE.TransformControlsGizmo = function () {
   ...
   // matRed.color.set(0xff0000);
    matRed.color.set(0x00ff00);
    var matGreen = gizmoMaterial.clone();
    //matGreen.color.set(0x00ff00);
    matGreen.color.set(0x0000ff);

    var matBlue = gizmoMaterial.clone();
    //matBlue.color.set(0x0000ff);
    matBlue.color.set(0xff0000);
   ...
   // matLineRed.color.set(0xff0000);
    matLineRed.color.set(0x00ff00);
    var matLineGreen = gizmoLineMaterial.clone();
    //matLineGreen.color.set(0x00ff00);
    matLineGreen.color.set(0x0000ff);
    var matLineBlue = gizmoLineMaterial.clone();
   // matLineBlue.color.set(0x0000ff);
    matLineBlue.color.set(0xff0000);
