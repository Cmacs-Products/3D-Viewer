...
var FileUpload = (function () {
    ...
    FileUpload.prototype = {
      ...
      afterUpload(document, input, dt) {
        var that = this;
        that.informServerAboutSuccessfulUpload(document).then(
          data => {
            try {
            
              document.Compressed = true;
              //data.Document.Compressed = true;
              that.finishedUploadProcessDocuments.push(data);
              if (typeof input.data !== 'undefined' && typeof input.data.isDocumentTemplate !== 'undefined' && input.data.isDocumentTemplate) {
                input.objectId = input.data;
              }
              //if($("#AttachHiddenDocumentDialogContainer").length === 0){
              input.attach(
                data,
                input.objectId,
                document,
                function () {
                  input.success(data, input.data, document);
                    input.complete(dt, input.data, Array.from(Object.values(that.finishedUploadProcessDocuments), x => x.Document));
                  //if (that.IsHiddenFile) uploadComplete(e);
                },
                function (e) {
                    input.complete(dt, input.data, Array.from(Object.values(that.finishedUploadProcessDocuments), x => x.Document));
